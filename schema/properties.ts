...
export const CreatePropertySchema = z.object({
  ...
  rooms: z.number().int().min(0), // At least one room
  baths: z.number().int().min(0), // Bathrooms, allow zero
  area: z.number().min(1), // Positive number for area (square meters or sqft)
  rentFrequency: RentFrequencyEnum.nullable(), // Optional field matching RentFrequency enum
  location: z.string().min(1).max(255), // Location string
  description: z.string().min(0), // Description must be non-empty
