...
export default function SearchPage() {
  ...
  const fetchProperties = async ( page: number, hitsPerPage: number) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/properties?purpose=for-rent&page=${page}`);
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {properties.map((property) => (
                    <div
                        key={property.id}
                        className="border rounded-lg overflow-hidden shadow-lg cursor-pointer flex flex-col"
